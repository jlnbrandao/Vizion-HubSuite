...
async def test_create_role_and_assign_valid_permissions(
    ...
    p1 = await create_perm.handle(CreatePermissionCommand(tenant_id=UNIVERSE_TENANT_ID,code="users.read", name="Read"))
    p2 = await create_perm.handle(CreatePermissionCommand(tenant_id=UNIVERSE_TENANT_ID,code="users.write", name="Write"))
    role_id = await create_role.handle(CreateRoleCommand(tenant_id=UNIVERSE_TENANT_ID,name="ADMIN", description="Full"))
...
async def test_assign_rejects_unknown_permission_ids(
    ...
    role_id = await create_role.handle(CreateRoleCommand(tenant_id=UNIVERSE_TENANT_ID,name="VIEWER"))
...
async def test_replace_and_revoke_permissions(
    ...
    a = await create_perm.handle(CreatePermissionCommand(tenant_id=UNIVERSE_TENANT_ID,code="a.read", name="A"))
    b = await create_perm.handle(CreatePermissionCommand(tenant_id=UNIVERSE_TENANT_ID,code="b.read", name="B"))
    c = await create_perm.handle(CreatePermissionCommand(tenant_id=UNIVERSE_TENANT_ID,code="c.read", name="C"))
    role_id = await create_role.handle(CreateRoleCommand(tenant_id=UNIVERSE_TENANT_ID,name="MANAGER"))
